...
object DwmTa {
  def runData(startDay: String, endDay: String): Unit = {
    ...
    val list: util.List[String] = DorisDBUtils.queryAppPkgConf(connDorisDB);
    DorisDBUtils.close()
    list.forEach(conf => {

      val confArr = conf.split("_");
      val taProjectId = confArr(0);
      val isEarningsEcpm = confArr(1);
      val isThingioData = confArr(2);
